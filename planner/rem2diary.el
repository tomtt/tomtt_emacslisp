...
(defcustom remind-planner-file "~/.reminders.planner"
   "Where reminders extracted from planner files are stored.
...
(defcustom remind-plan2rem "plan2rem"
  "The path of the planner to remind converter."
  :type 'file
  :group 'remind)
...
(defun remind-parse-planner ()
   "Extract all appointments from planner files."
   (interactive)
   (with-current-buffer (find-file-noselect remind-planner-file)
     (erase-buffer)
     (call-process remind-plan2rem nil t nil (expand-file-name planner-directory))
     (save-buffer)))
